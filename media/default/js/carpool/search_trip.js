...
function retrieveTrips() {
    if (trip_type == TYPE_OFFER || trip_route != null) {
        $('wait').show();
        new Ajax.Request('/ajax/get_trips/', { method:'post',
            parameters: {
                departure_name: $('id_departure_name').getValue(),
                departure_point: wkt.write(
                        departure_marker
                    ),
                departure_favoriteplace: $('id_departure_favoriteplace').getValue(),
                departure_sync: departure_sync,
                arrival_name: $('id_arrival_name').getValue(),
                arrival_point: wkt.write(
                        arrival_marker
                    ),
                arrival_favoriteplace: $('id_arrival_favoriteplace').getValue(),
...
if ($('form_save_trip')) {
    $('form_save_trip').observe('submit', function(event) {
        var departure = $H({
            ...
            'favoriteplace': $('id_departure_favoriteplace').getValue()
        });
        var arrival = $H({
            'city': $('id_arrival_name').getValue(),
            'point': wkt.write(arrival_marker),
            'favoriteplace': $('id_arrival_favoriteplace').getValue()
        });
        var trip_details = $H({
