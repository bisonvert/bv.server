...
function displayTrip() {
    ...
    if (getTripType() != TYPE_DEMAND && linestring_array.length > 0) {
        ...
        if (isRouteOK()) {
            calculateSimpleTripBuffer(trip_pls, map, trip_offer_radius, null, trip_layer);
        }
    }
}
...
function retrieveTrips() {
    if (getTripType() == TYPE_DEMAND || trip_pls != null) {
        if (!isRouteOK()) return;
        displayWaitForResults();
        new Ajax.Request('/ajax/get_trips/' + trip_id + '/', { method:'post',
            parameters: {
            ...
            onSuccess: function(transport){
                ...
                if (getTripType() != TYPE_DEMAND)
                    trip_demands_updater.updateTrips(json.trip_demands, json.authenticated);
                if (getTripType() != TYPE_OFFER)
                    trip_offers_updater.updateTrips(json.trip_offers, json.authenticated);
            },
            onFailure: function() {
                hideWaitForResults();
            }
...
function displayWaitForResults() {
    $('wait').show();
}
function hideWaitForResults() {
    $('wait').hide();
}
...
function isRouteOK() {
    if (getTripType() == TYPE_DEMAND) return true;
...
$('permalink_field').observe('click', function() {
    $('permalink_field').select();
});

if (getTripType() == TYPE_BOTH) {
    $('res_o').observe('click', function() {
        Element.extend($('res_o'));
        Element.extend($('res_d'));
        $('trip_list_content_d').hide();
        $('trip_pages_d').hide();
        $('res_d').removeClassName('current');
        $('trip_list_content_o').show();
        $('trip_pages_o').show();
        $('res_o').addClassName('current');
    });
    $('res_d').observe('click', function() {
        Element.extend($('res_o'));
        Element.extend($('res_d'));
        $('trip_list_content_o').hide();
        $('trip_pages_o').hide();
        $('res_o').removeClassName('current');
        $('trip_list_content_d').show();
        $('trip_pages_d').show();
        $('res_d').addClassName('current');
    });
}
